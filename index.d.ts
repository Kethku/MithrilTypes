...
declare namespace Mithril {
		...
		type Child = string | VirtualElement | Component<any>;
		...
		interface Static {
				 ...
				 */
				request<Data, Res, Ret>(options: RequestOptions<Data, Res, Ret>): Promise<Ret>
		...
		interface LifecycleMethods {
				oninit(vnode:VirtualElement);
				oncreate(vnode:VirtualElement);
				onupdate(vnode:VirtualElement);
				onbeforeremove(vnode:VirtualElement);
				onremove(vnode:VirtualElement);
				onbeforeupdate(vnode:VirtualElement, old:VirtualElement): boolean;
		}
		...
		interface RequestOptions<Data, Res, Ret> {
				 ...
				 */
				serialize?(dataToSerialize: Data): string;
				 ...
				 */
				deserialize?(dataToDeserialize: string): Res;
